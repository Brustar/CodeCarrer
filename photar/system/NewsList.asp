...
<%
    ...
    cateid = request("ID")
	if cateid=6 and session("language")=1 then cateid=4
	if cateid=7 and session("language")=1 then cateid=8
    thispage=1
...
function goPage(page){
	document.Pageform.action="NewsList.asp?thispage="+page;
      ...
      <table border="0" cellspacing="0" cellpadding="0" width="100%">
        <tr>
          <td bgcolor="#0C89A7"  valign="middle" >
            <p class="unnamed1" align="center">新 闻 信 息 管 理 </td>
        </tr>
      </table>
    </td>
  </tr>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <form name="FrmNews" method="post">
    ...
    <tr>
      <td class="cpx12black" height=65>&nbsp;标题：
...
<select name="id">
<option></option>
<%getSelectTree "NewsCate","Catename","",session("language")%>
</SELECT>
                ...
                <a href="NewsInfoEdit.asp?Mode=NewInfo&CateID=<%=CateID%>" class="cpx12black"><img src="../images/button_add.gif" border="0" alt="新增" align="absmiddle"></a>&nbsp;&nbsp;
<table border="0" cellspacing="8" cellpadding="0" width="100%">
                  <tr>
                    <td width="100%" bgcolor="#646464" colspan="2">
                      <table border="0" width="100%" cellpadding="3" cellspacing="1">
                        <tr bgcolor="#C8C8C8" align=center>
                          <td width="288" class="cpx12black">标题</td>
                          <td width="121" class="cpx12black">类别</td>
                          <!--<td width="100" class="cpx12black">浏览级别</td>-->
                          <td width="182" class="cpx12black">来源(出处)</td>
...
'页面显示
	...
	sql="select * from Newsinfo where 1=1"
	if TITLE<>"" then sql=sql & " and subject like '%" & TITLE & "%'"
	...
	if not rs.eof then
		...
		if thispage>rs.PageCount then thispage=rs.PageCount
   		for i=1 to rs.pagesize  
			...
			subject=rs("subject")
			CateID=rs("cateid")
			cateName=getdata("cateName","newscate","id",CateID)
			resource=rs("source")
...
<tr bgcolor="#FFFFFF" align=left>
  <td width="288">
    <a href="NewsInfoEdit.asp?Mode=EditInfo&NEWSID=<%=ID%>&CateID=<%=CateID%>" class="cpx12black">
    <%=subject%></a>
  </td>
