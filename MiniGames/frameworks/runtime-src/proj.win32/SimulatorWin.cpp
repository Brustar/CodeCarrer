...
int CopyFolder(const char* lpszFromPath, const char* lpszToPath)
{
	...
	//system("xcopy \"$(ProjectDir)..\\..\\..\\res\" \"$(OutDir)\\Resource\\res\" /e /Y");
	fop.wFunc = FO_COPY;
	fop.pFrom = NewPathFrm;//Srcstr.c_str(); 
	fop.pTo = lpszToPath;//Dststr.c_str(); 
	fop.fFlags = FOF_SILENT | FOF_NOCONFIRMATION | FOF_NOERRORUI | FOF_NOCONFIRMMKDIR;
	int i = SHFileOperationA(&fop);
	delete[]NewPathFrm;
	return i;
}
#endif
...
{
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)//copy 文件夹然后重新执行EXE
	char buf[MAX_PATH] = { 0 };
	GetCurrentDirectoryA(MAX_PATH, buf);  //得到当前工作路径

	char DstPath[MAX_PATH] = { 0 };
	char SrcPath[MAX_PATH] = { 0 };
	sprintf_s(DstPath, MAX_PATH, ("%s\\"), buf);
	sprintf_s(SrcPath, MAX_PATH, ("%s\\..\\..\\src"), buf);
	int i = CopyFolder(SrcPath, DstPath);

	sprintf_s(DstPath, MAX_PATH, ("%s\\"), buf);
	sprintf_s(SrcPath, MAX_PATH, ("%s\\..\\..\\res"), buf);
	int j = CopyFolder(SrcPath, DstPath);

	TCHAR moduleName[MAX_PATH];
	ZeroMemory(moduleName, sizeof(moduleName));
	GetModuleFileName(NULL, moduleName, MAX_PATH);
	//CCLog("Srcstr = %s Dststr = %s  src = %d res =%d \n", SrcPath, DstPath, i, j);
...
void SimulatorWin::setupUI()
{
    ...
    menuBar->addItem("REFRESH_MENU", tr("Refresh\tF5"), "VIEW_MENU");
