    ...
    zzip_plugin_io_t io)
{
    ...
    char* fd_map = 0; 
    int32_t  fd_gap = 0;
	uint16_t u_entries = ZZIP_GET16((unsigned char*)trailer->z_entries);
	uint32_t u_rootsize = ZZIP_GET32((unsigned char*)trailer->z_rootsize);
	uint32_t u_rootseek = ZZIP_GET32((unsigned char*)trailer->z_rootseek);
    ...
    for (entries=u_entries, offset=0; entries > 0; entries--)
    {
        ...
        if (fd_map) 
		{
			d = (zzip_r_dir*)(fd_map + fd_gap + offset);
...
#       endif        
		...
		u_extras = ZZIP_GET16((unsigned char*)d->z_extras);
		u_comment = ZZIP_GET16((unsigned char*)d->z_comment);
		u_namlen = ZZIP_GET16((unsigned char*)d->z_namlen);
		...
		hdr->d_crc32 = ZZIP_GET32((unsigned char*)d->z_crc32);
		hdr->d_csize = ZZIP_GET32((unsigned char*)d->z_csize);
		hdr->d_usize = ZZIP_GET32((unsigned char*)d->z_usize);
		hdr->d_off = ZZIP_GET32((unsigned char*)d->z_off);
		hdr->d_compr = (uint8_t)ZZIP_GET16((unsigned char*)d->z_compr);
...
__zzip_dir_parse (ZZIP_DIR* dir)
{
    ...
    HINT5("directory = { entries= %d/%d, size= %d, seek= %d } ", 
	  ZZIP_GET16((unsigned char *)trailer.z_entries),  ZZIP_GET16((unsigned char *)trailer.z_finalentries),
	  ZZIP_GET32((unsigned char *)trailer.z_rootsize), ZZIP_GET32((unsigned char *)trailer.z_rootseek));
