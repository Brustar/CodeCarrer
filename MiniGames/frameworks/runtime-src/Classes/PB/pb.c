...
static int mergeData(lua_State* L)
{
	unsigned short mainCmdID=luaL_checkinteger(L,1);
	unsigned short subCmdID = luaL_checkinteger(L,2);
	const char* sBuffer = luaL_checkstring(L, 3);
	int nLen = strlen(sBuffer);
	
	char buffer[SOCKET_TCP_BUFFER] = { 0 };
	int length = HEADER_SIZE + nLen;
	int msgID = 2;
	if (sBuffer)
		memcpy(buffer + HEADER_SIZE, sBuffer, nLen);
	setPackInfo((byte*)buffer, length, msgID, mainCmdID, subCmdID);
	encryBuffer((byte*)buffer, length);
	lua_pushlstring(L,buffer,length);
	return 1;
}

static int splitData(lua_State* L)
{
	const char* m_pRecvBuff = luaL_checkstring(L, 1);
	int nPackLen = getPackSize((byte*)m_pRecvBuff, 0);
	decryptBuffer((byte*)m_pRecvBuff, 0, nPackLen);
	short mainCmdID = getMainCommand((byte*)m_pRecvBuff, 0);
	short subCmdIDD = getSubConmmand((byte*)m_pRecvBuff, 0);

	lua_pushinteger(L, mainCmdID);
	lua_pushinteger(L, subCmdIDD);
	lua_pushlstring(L, m_pRecvBuff + HEADER_SIZE, nPackLen - HEADER_SIZE);
	return 3;
}
