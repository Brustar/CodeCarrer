#include "EncryDecry.h"

#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define		DATA_MAPPED		 0x05   //0x01

/** ��֤�� **/
static const char * Validate= "C3D277CC-C992-4933-B015-C12F0DB58175";

static const int		USB_KN_VALIDATE_SOCKET			= 2;
static const int		SIZE_VALIDATE		            = 136;

//发送映射
const byte g_SendByteMap[256]=
{
	
};

//接收映射
const byte g_RecvByteMap[256]=
{
	
};

//加密密钥
const unsigned int g_dwPacketKey = 0xA55AA55A;

char mapsendvbyte(byte data)
{
	int len = (int)data;
	if (len >= 256)
	{
		len = len % 256;
	}
	return g_SendByteMap[len];
}

char maprecvbyte(byte data)
{
	return g_RecvByteMap[data];
}

int encryBuffer(byte* data, int dataSize)
{
	int checkCode = 0;
	int i = 6;
	for (i = 6; i < dataSize; i++) 
	{
		if (data[i] < 0) 
		{
			checkCode += (256 +  data[i]);
			data[i] = mapsendvbyte(256 +  data[i]);
		}
		else 
		{
			checkCode +=  data[i];
			data[i] = mapsendvbyte(data[i]);
		}

		checkCode %= 256;
	}

	data[0] |= DATA_MAPPED;
	data[1] = (char) (~checkCode + 1);

    return 1;
}

int decryptBuffer(byte* data, int start, int dataSize)
{
  	int checkCode = data[start + 1];
	int i = start + 6;
	for (; i < start + dataSize; i++) 
	{
		if (data[i] < 0) 
		{
			data[i] = maprecvbyte(256 + data[i]);
			checkCode += 256 + data[i];
		} 
		else 
		{
			data[i] = maprecvbyte(data[i]);
			checkCode += data[i]; 
		}
	}

	return 1;
}

byte getciphermode()
{
  	return DATA_MAPPED;
}

int getMainCommand(byte* data, int start)
{
	int height = data[start+7];
	int low = data[start+6];

	return (((height& 0xff) << 8) | (low& 0xff));
}

int getSubConmmand(byte* data, int start)
{
	int height = data[start+9];
	int low = data[start+8];

	return (((height& 0xff) << 8) | (low& 0xff));
}


int getPackSize(byte* data, int start)
{
	int height = data[start+3];
	int low = data[start+2];
	return (((height& 0xff) << 8) | (low& 0xff));
}

int setPackSize(byte* data, int nSize)
{
	data[2] = (byte) (nSize & 0xff);
	data[3] = (byte) ((nSize >> 8) & 0xff);
	return 1;
}

int getMsgID(byte* data, int start)
{
	int height = data[start+5];
	int low = data[start+4];
	return (((height& 0xff) << 8) | (low& 0xff));
}

void setPackInfo(byte* data, int dataSize, int nMsgID, int main, int sub)
{
	data[2] = (byte) (dataSize & 0xff);
	data[3] = (byte) ((dataSize >> 8) & 0xff);

	data[4] = (byte) (nMsgID & 0xff);
	data[5] = (byte) ((nMsgID >> 8) & 0xff);

	data[6] = (byte) (main & 0xff);
	data[7] = (byte) ((main >> 8) & 0xff);

	data[8] = (byte) (sub & 0xff);
	data[9] = (byte) ((sub >> 8) & 0xff);
}

int getCipher(byte* dataBuffer, int start)
{
	int cipher = dataBuffer[start];
	return cipher;
}

int getCipherCode(byte* dataBuffer, int start)
{
	int ciphercode = dataBuffer[start+1];
	return ciphercode;
}