...
function MainScene:onData(__event)
    print("socket receive raw data:", __event.data,#__event.data)
    local cmd,subcmd,pbstring=encoder.splitData(__event.data)
    local obj=ServerStruct_pb.PBGameUpgrade()
    obj:ParseFromString(pbstring)
    print(obj.DownUrl)
end
...
function MainScene:initSocket()
    if not self._socket then
        self._socket = SocketTCP.new(TCP_IP, TCP_PORT, false)
