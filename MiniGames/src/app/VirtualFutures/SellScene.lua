...
local SellScene = class("SellScene", 
	function()
		return cc.Scene:create()
	end)

function SellScene:ctor(typeid)
	self.typeid = typeid
...
function SellScene:initWithCSB()
    local node = cc.CSLoader:createNode("sell_weituo.csb")
    self:addChild(node)
	...
	self.btn_sell = pl_weituo:getChildByName("btn_sell")
	self.btn_sell:addClickEventListener(function(sender, eventType)
		self:Commit()
	end)
end

function SellScene:getMaxTradeNum()
	return math.floor(self.myqh.tradenum)
end

function SellScene:getTradeNum()
	return tonumber(self.TextField_3:getString())
end

function SellScene:setTradeNum(num)
	self.TextField_3:setString(num)
end

function SellScene:setTradeNumPercent(percent)
	self:setTradeNum(math.floor(self:getMaxTradeNum() * percent / 100))
end

function SellScene:loadTypeDetail()
...
function SellScene:loadMyqhDetail()
    ...
    self.text_:setString("最大卖出数量" .. self:getMaxTradeNum() .."股")
end

function SellScene:Commit()
	local op = 2
	...
	print(pbResponse.msg, pbResponse.userstone)
end
